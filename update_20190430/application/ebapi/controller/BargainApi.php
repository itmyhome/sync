...
class BargainApi extends AuthController
{
    ...
    public function bargain_share_poster()
    {
        ...
        try{
            ...
            $path = makePathToUrl('routine/activity/bargain/code',3);
            if($path == '') return JsonService::fail('生成上传目录失败,请检查权限!');
            $codePath = $path.$bargainId.'_'.$this->userInfo['uid'].'.jpg';
            ...
            $data['url'] = $codePath;
            $path = makePathToUrl('routine/activity/bargain/poster',3);
            if($path == '') return JsonService::fail('生成上传目录失败,请检查权限!');
            $filename = ROOT_PATH.$path.'/'.$bargainId.'_'.$this->userInfo['uid'].'.jpg';
        ...
        $poster = $domain.$path.'/'.$bargainId.'_'.$this->userInfo['uid'].'.jpg';
