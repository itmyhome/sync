...
Page({
  ...
  goPay: function () {//立即支付
    ...
    var data = { 
      ...
      cartNum: that.data.bargainInfo.bargain_num,
      uniqueId:'',
      combinationId:0,
      secKillId:0,
    };
  ...
  getBargainDetails:function(){//获取砍价产品详情
    ...
    app.basePost(app.U({ c: 'bargain_api', a:'get_bargain'}),data,function(res){
      that.setData({ bargainInfo: res.data.bargain, userInfo: res.data.userInfo });
  ...
  getBargainHelpCount: function () {//获取砍价帮总人数、剩余金额、进度条、已经砍掉的价格
    ...
    app.basePost(app.U({ c: 'bargain_api', a: 'get_bargain_help_count' }), data, function (res) {
      that.setData({ 
        ...
        'bargainInfo.price': util.$h.Sub(that.data.bargainInfo.price, res.data.alreadyPrice)
      });
    }, function (res) { console.log(res); })
  },
